...
def admin_bookings():
    ...
    bookings = get_all_bookings(mysql, status_filter)
...
def toggle_hall(hall_id):
    ok, msg = toggle_hall_status(mysql, hall_id)
    flash(msg, 'success' if ok else 'danger')
...
def toggle_faculty(user_id):
    ok, msg = toggle_faculty_status(mysql, user_id)
    flash(msg, 'success' if ok else 'danger')
